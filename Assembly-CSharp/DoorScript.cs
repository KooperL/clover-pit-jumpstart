...
public class DoorScript : MonoBehaviour
{
	...
	{
		if (Data.game.jumperinoScarinoDoorino_DoneOnce && Random.value > 0.3f)
		{
			return;
		}
	...
	{
		float timer = Random.Range(1.75f, 3f);
		while (timer > 0f)
		{
		...
		while (timer > 0f)
		{
			timer -= Tick.Time;
			yield return null;
			base.transform.position = this.startPos + new Vector3(Random.Range(-timer / 2f, timer / 2f), Random.Range(-timer / 2f, timer / 2f), Random.Range(-timer / 2f, timer / 2f));
		}
		CameraGame.Shake(2f);
		timer = 0.25f;
		while (timer > 0f)
